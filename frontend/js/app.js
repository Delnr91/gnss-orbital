...
const translations = {
        ...
        "overview.title": "APEX-1 Orbital Operations",
        "overview.subtitle": "Interactive Keplerian Trajectory Planning & Space Second Brain",
        "overview.briefing_title": "MISSION BRIEFING",
        "overview.briefing_text": "Welcome to the interactive training suite. This system models and solves Keplerian two-body orbital mechanics. It is designed to train space engineering students in orbital parameters (LEO, MEO, GEO, HEO), satellite constellation tracking, and non-Keplerian perturbations (J2 effect). Our local core covers next-gen 2030 GNSS and the EU's IRISÂ² multi-orbital architectures.",
        "overview.constellation_title": "CONSTELLATION STATUS MONITOR",
        "overview.constellation_desc": "Live active satellite fleets telemetry and synchronization parameters in the Earth-Centered Inertial (ECI) frame.",
        "overview.constellation.gps": "GPS III (USA)",
        "overview.constellation.galileo": "Galileo (EU)",
        "overview.constellation.beidou": "BeiDou-3 (CN)",
        "overview.constellation.iris": "IRISÂ² Hybrid (EU)",
        "overview.status.nominal": "NOMINAL",
        "overview.status.syncing": "SYNC ACTIVE",
        "overview.constellation.altitude": "Altitude",
        "overview.constellation.planes": "Planes",
        "sim.title": "3D Orbit Simulator",
        "sim.subtitle": "Propagate Keplerian trajectories and analyze satellite speeds in real-time.",
        "warning.subsurface": "Orbit intersects Earth's surface! Perigee is below the planetary radius ({radius} km).",
        "ctrl.constellation_exit": "◈ EXIT MULTI-ORBIT",
    ...
    es: {
        ...
        "overview.title": "Operaciones Orbitales APEX-1",
        "overview.subtitle": "Planificación Interactiva de Trayectorias Keplerianas y Segundo Cerebro Espacial",
        "overview.briefing_title": "BRIEFING DE MISIÓN",
        "overview.briefing_text": "Bienvenido a la suite de entrenamiento interactiva. Este sistema modela y resuelve la mecánica orbital kepleriana de dos cuerpos. Está diseñado para entrenar a estudiantes de ingeniería espacial en parámetros orbitales (LEO, MEO, GEO, HEO), seguimiento de constelaciones de satélites y perturbaciones no keplerianas (efecto J2). Nuestro núcleo local cubre el GNSS de próxima generación 2030 y las arquitecturas multi-orbitales IRIS² de la UE.",
        "overview.constellation_title": "MONITOR DE ESTADO DE CONSTELACIONES",
        "overview.constellation_desc": "Telemetría en vivo de flotas de satélites activas y parámetros de sincronización en el marco inercial centrado en la Tierra (ECI).",
        "overview.constellation.gps": "GPS III (EE.UU.)",
        "overview.constellation.galileo": "Galileo (UE)",
        "overview.constellation.beidou": "BeiDou-3 (CN)",
        "overview.constellation.iris": "IRIS² Híbrida (UE)",
        "overview.status.nominal": "NOMINAL",
        "overview.status.syncing": "SINC. ACTIVA",
        "overview.constellation.altitude": "Altitud",
        "overview.constellation.planes": "Planos",
        "sim.title": "Simulador Orbital 3D",
        "sim.subtitle": "Propaga trayectorias keplerianas y analiza velocidades de satélites en tiempo real.",
        "warning.subsurface": "¡La órbita intersecta la superficie terrestre! El perigeo está por debajo del radio planetario ({radius} km).",
        "ctrl.constellation_exit": "◈ SALIR DE MULTI-ÓRBITA",
    ...
    zh: {
        ...
        "overview.title": "APEX-1 轨道任务中心",
        "overview.subtitle": "交互式开普勒轨迹规划与太空第二大脑",
        "overview.briefing_title": "任务简报",
        "overview.briefing_text": "欢迎使用交互式训练套件。本系统建模并求解开普勒二体轨道力学，旨在训练航天工程学员掌握轨道参数（LEO、MEO、GEO、HEO）、卫星星座跟踪以及非开普勒摄动（J2 效应）。我们的本地核心涵盖下一代 2030 GNSS 与欧盟 IRIS² 多轨道架构。",
        "overview.constellation_title": "星座状态监视器",
        "overview.constellation_desc": "在地心惯性（ECI）坐标系中实时监测现役卫星舰队的遥测与同步参数。",
        "overview.constellation.gps": "GPS III（美国）",
        "overview.constellation.galileo": "伽利略（欧盟）",
        "overview.constellation.beidou": "北斗三号（中国）",
        "overview.constellation.iris": "IRIS² 混合星座（欧盟）",
        "overview.status.nominal": "正常",
        "overview.status.syncing": "同步中",
        "overview.constellation.altitude": "高度",
        "overview.constellation.planes": "轨道面",
        "sim.title": "3D 轨道模拟器",
        "sim.subtitle": "实时传播开普勒轨迹并分析卫星速度。",
        "warning.subsurface": "轨道与地球表面相交！近地点低于行星半径（{radius} km）。",
        "ctrl.constellation_exit": "◈ 退出多轨道视图",
        "ctrl.presets": "预设轨道",
        "preset.custom": "自定义",
        "preset.leo": "低地轨道 (ISS)",
        "preset.meo": "中地轨道 (GPS)",
...
function toggleConstellation() {
    ...
    if (isConstellationMode) {
        ...
        if (btn) btn.innerText = translations[currentLang]["ctrl.constellation_exit"] || "◈ EXIT MULTI-ORBIT";
