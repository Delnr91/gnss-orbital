...
KNOWLEDGE_BASE = {
    "en": {
        ...
        "jupyter": "To completely bypass the Jupyter password/token prompt, run this exact command in your terminal:\n\n```bash\njupyter lab --no-browser --NotebookApp.token=''\n```\n\nThis forces Jupyter to start without any authentication. After running it, simply click 'OPEN IN TAB' in the APEX-1 console.",
        "password": "To completely bypass the Jupyter password/token prompt, run this exact command in your terminal:\n\n```bash\njupyter lab --no-browser --NotebookApp.token=''\n```\n\nThis forces Jupyter to start without any authentication. After running it, simply click 'OPEN IN TAB' in the APEX-1 console.",
    ...
    "es": {
        ...
        "jupyter": "Para saltarte completamente la contraseña de Jupyter, ejecuta este comando exacto en tu terminal:\n\n```bash\njupyter lab --no-browser --NotebookApp.token=''\n```\n\nEsto fuerza a Jupyter a iniciar sin autenticación. Luego, simplemente haz clic en 'OPEN IN TAB' en la consola APEX-1.",
        "contraseña": "Para saltarte completamente la contraseña de Jupyter, ejecuta este comando exacto en tu terminal:\n\n```bash\njupyter lab --no-browser --NotebookApp.token=''\n```\n\nEsto fuerza a Jupyter a iniciar sin autenticación. Luego, simplemente haz clic en 'OPEN IN TAB' en la consola APEX-1.",
    ...
    "zh": {
        ...
        "jupyter": "要完全跳过 Jupyter 密码/令牌提示，请在终端中运行以下确切命令：\n\n```bash\njupyter lab --no-browser --NotebookApp.token=''\n```\n\n这会强制 Jupyter 在没有身份验证的情况下启动。运行后，只需在 APEX-1 控制台中点击 'OPEN IN TAB' 即可。",
        "密码": "要完全跳过 Jupyter 密码/令牌提示，请在终端中运行以下确切命令：\n\n```bash\njupyter lab --no-browser --NotebookApp.token=''\n```\n\n这会强制 Jupyter 在没有身份验证的情况下启动。运行后，只需在 APEX-1 控制台中点击 'OPEN IN TAB' 即可。",
